...
func FetchUserRides(c *fiber.Ctx) error {
	...
	if userIDParam == "" {
		return &fiber.Error{Code: 400, Message: "Missing user ID parameter"}
	}

	
	userUUID, parseErr := uuid.Parse(userIDParam)
	if parseErr != nil {
		return &fiber.Error{Code: 400, Message: "Invalid UUID format"}
	}

	var userRides []UserRidesResponse
